...
* `coef_popularidad`: Aplica una transformación logarítmica y una función sigmoide para suavizar los extremos. Esto evita que un artista con billones de reproducciones rompa la escala de precios, manteniendo el multiplicador acotado de forma segura entre `1.0` y `2.0`. Para entender porque se tomó la decision de usar esta funcion vea el documento "[analisis y documentacion del coeficiente](documentacion_concefeccion_popularidad.md)" y puede ver el proceso de pensamiento en el documento interactivo [Proceso de confeccion del coeficiente](../Basurero_de_ideas.ipynb).
